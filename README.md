...
Those tools are optional, but highly recommended:
...
```
docker run --name minikube-api-raw --net host ghcr.io/cybercryptio/slu:v0.71.0 slu proxy tcp -l :8443 -r $(minikube ip):8443
```

See: <https://lab0.cyber-crypt.com:8443>

or:

```
curl -k https://lab0.cyber-crypt.com:8443
```
...
```
docker run -d --name proxy --net host -v /root:/root ghcr.io/cybercryptio/dev kubectl proxy --address 0.0.0.0 --accept-hosts=".*"
```
...
See credentials (of example `registry.gitlab.com`)

```
kubectl get secret private-registry-credentials -o jsonpath="{.data.\.dockerconfigjson}" | base64 --decode | jq '.auths["registry.gitlab.com"].auth' -r | base64 --decode && echo
```
...
```
kubectl run dev -ti --rm --image=ghcr.io/cybercryptio/dev -- bash
```
...
```
kubectl run dev -it --rm --image=ghcr.io/cybercryptio/dev -- bash
```
...
Run `ghcr.io/cybercryptio/dev`:

```
alias kdev='kubectl run "dev-$(date +%s)" --rm -ti --image ghcr.io/cybercryptio/dev -- bash'
```
...
```
docker run -d --name proxy-ingress-80 --net host -v /root:/root ghcr.io/cybercryptio/slu:v0.71.0 slu proxy tcp -l :80 -r $(kubectl get node minikube -o jsonpath="{.status.addresses[0].address}"):$(kubectl get svc -n ingress-nginx ingress-nginx-controller -o jsonpath="{.spec.ports[0].nodePort}")
```

```
docker run -d --name proxy-ingress-443 --net host -v /root:/root ghcr.io/cybercryptio/slu:v0.71.0 slu proxy tcp -l :443 -r $(kubectl get node minikube -o jsonpath="{.status.addresses[0].address}"):$(kubectl get svc -n ingress-nginx ingress-nginx-controller -o jsonpath="{.spec.ports[1].nodePort}")
```
...
- http://nginx.k8s.cyber-crypt.com
- http://apache.k8s.cyber-crypt.com
...
Example with my NFS server (nfs.cyber-crypt.com):

```
helm install nfs-subdir-external-provisioner nfs-subdir-external-provisioner/nfs-subdir-external-provisioner --namespace nfs-subdir-external-provisioner --create-namespace --set nfs.server=nfs.cyber-crypt.com --set nfs.path=/nfs
```
...
```
kubectl run ab --image=ghcr.io/cybercryptio/dev --rm -ti -- ab -c 4 -n 100000 http://hpa-service/
```
...
```
helm repo add cybercrypt cybercrypt https://cybercrypt-helm.vercel.app
```
...
```
helm search repo cybercrypt
```

### Inspect Package

```
helm show chart cybercrypt/hello-world
helm show values cybercrypt/hello-world
helm show readme cybercrypt/hello-world

helm show all cybercrypt/hello-world
```
...
Example [hello-world](https://github.com/cybercryptio/charts/tree/master/charts/hello-world) (on [ArtifactHub](https://artifacthub.io/packages/helm/cybercrypt/hello-world)) package / chart:

```
helm install hello cybercrypt/hello-world
```

Or dry run (see the Kubernetes config)

```
helm install hello cybercrypt/hello-world --dry-run
```

## Install Package Without Helm Repo Add

```
helm install hello --repo https://cybercrypt-helm.vercel.app hello-world
```
...
```
helm upgrade hello cybercrypt/hello-world --set replicas=3
```

or

```
helm upgrade hello cybercrypt/hello-world --set host=hello.127.0.0.1.nip.io
```
...
```
helm upgrade --install hello cybercrypt/hello-world --set host=hello.127.0.0.1.nip.io
```
...
#### Install cybercrypt/one-image

Sources of Chart `cybercrypt/one-image` are <https://github.com/cybercryptio/charts/tree/master/charts/one-image>

Inspect Package

```
helm show all cybercrypt/one-image
```

Install with values in args

```
helm install hello-world cybercrypt/one-image --set host=hello-world.127.0.0.1.nip.io
```

Install with values file on Minikube

```
helm install nginx cybercrypt/one-image --values one-image-nginx-values.yml
helm install apache cybercrypt/one-image --values one-image-apache-values.yml
helm install caddy cybercrypt/one-image --values one-image-caddy-values.yml
```

Install with values file and values args

```
helm install nginx2 cybercrypt/one-image --values one-image-nginx-values.yml --set host=nginx2.127.0.0.1.nip.io
```

Install with values file on DigitalOcean

```
helm install nginx cybercrypt/one-image \
  -f one-image-nginx-values.yml \
  -f one-image-nginx-values-do.yml
helm install apache cybercrypt/one-image \
  -f one-image-apache-values.yml \
  -f one-image-apache-values-do.yml
helm install caddy cybercrypt/one-image \
...
This default manifests are too much complicated, if you want simple examle, check out my [cybercrypt/one-image](https://github.com/cybercryptio/charts/tree/master/charts/one-image).
...
```
scp repo/* root@cybercrypt-helm-demo.vercel.app:/helm/
```
...
```
helm repo add cybercrypt cybercrypt-demo https://cybercrypt-helm-demo.vercel.app
```

Install package

```
helm install counter cybercrypt-demo/counter --set host=counter.127.0.0.1.nip.io
```
...
or with TLS SAN (for example for `k3s.cyber-crypt.com`)

```
curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="server --disable traefik --tls-san k3s.cyber-crypt.com" sh
```
